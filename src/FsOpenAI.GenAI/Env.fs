...
module Env =
            ...
            let! settingsText =
                let path =
                    if not(String.IsNullOrWhiteSpace config.SettingsFile) then
                        let path = Path.Combine(Utils.homePath.Value,Environment.ExpandEnvironmentVariables(config.SettingsFile))
                        path
                    else
                        ""
    ...
    let loadConfig() =
        try
            let path = Path.GetFullPath(Path.Combine(wwwRootPath(),C.APP_CONFIG_PATH.Value))
