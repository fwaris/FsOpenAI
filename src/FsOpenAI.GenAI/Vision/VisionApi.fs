...
let processVision (ep:Uri) (key:string) (user:string) (payload:Payload) =
    task {
        ...
        client.DefaultRequestHeaders.Add(HttpHeader.Names.UserAgent, user)
...
let processImage (parms:ServiceSettings) (invCtx:InvocationContext) (backend:Backend) (modelsConfig:ModelsConfig) (sysMsg:string, userPrompt:string, img:byte[]) =
    match GenUtils.visionModel backend modelsConfig with
    | Some model ->
        async {
