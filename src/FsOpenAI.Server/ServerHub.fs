    ...
    let initClient sttngs dispatch =
        task {
            try            
                ...
                | Some cfg -> 
                    let metaIndex = cfg.MetaIndex |> Option.defaultValue C.DEFAULT_META_INDEX
                    do! initIndexes sttngs cfg.IndexGroups metaIndex dispatch
    ...
    member this.FromClient(msg:ClientInitiatedMessages) : Task = 
        ...
        task{
            try 
                ...
                | Clnt_ProcessChat (settings,modelsConfig,chat) ->
                    let settings = Settings.updateKey settings                       
                    if (Interaction.cBag chat).UseWeb then 
                        WebCompletion.processWebChat settings modelsConfig chat dispatch |> Async.Start
                    else
                        Completions.streamCompleteChat settings modelsConfig chat dispatch |> Async.Start
                ...
                | Clnt_ProcessQA (settings,modelsConfig,chat) ->
                    let settings = Settings.updateKey settings
                    let dispatch msg = ServerHub.SendMessage(client,msg) |> ignore
                    QnA.runPlan settings modelsConfig chat dispatch |> Async.Start

                | Clnt_ProcessDocQA (settings,modelsConfig,chat) ->
                    let settings = Settings.updateKey settings
                    DocQnA.runPlan settings modelsConfig chat dispatch |> Async.Start
                ...
                | Clnt_SearchQuery (settings,modelsConfig,ch) -> 
                    let settings = Settings.updateKey settings
                    do! DocQnA.extractQuery settings modelsConfig ch dispatch
