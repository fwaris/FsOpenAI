...
//      | <handle typed exceptions>
    ...
    let update (uparms:UpdateParms) message model =
        //printfn "%A" message
        match message with
        | StartInit -> {model with busy=true}, Cmd.OfTask.either Init.pingServer (uparms.serverConnect,uparms.serverDispatch) LoadUIState Error
        ...
        *)
        ...
        //session and state
        | GetInitConfig -> model,Cmd.OfTask.either uparms.getInitConfig () GotInitConfig Error
        | GotInitConfig c -> {model with loadConfig=c},(if c.RequireLogin then Cmd.none else Cmd.ofMsg StartInit)
        | CloseBanner _ -> uparms.dialogService.Close(); model, Cmd.none
        ...
        | LoadUIState _ -> model,Cmd.OfTask.either IO.loadUIState uparms.localStore LoadedUIState Error
        ...
        | FromServer (Srv_Ia_Thought (id,tht)) -> {model with interactions = Interactions.setThought id tht model.interactions},Cmd.none
        ...
        | FromServer (Srv_Info msg) -> model,Cmd.ofMsg (ShowInfo msg)
