...
module ClientHub =
    ...
    let send clientDispatch (conn:Ref<HubConnection option>) (msg:ClientInitiatedMessages) = 
        match conn.Value with 
        | Some conn -> _send C.ClientHub.fromClient clientDispatch conn msg
        | None -> printfn "no hub connection"

    let call (conn:Ref<HubConnection option>) (msg:ClientInitiatedMessages) =
        match conn.Value with 
        | Some conn -> conn.SendAsync(C.ClientHub.fromClient,msg)
        | None -> printfn "no hub connection"; Threading.Tasks.Task.CompletedTask
