    ...
    let runPlan (parms:ServiceSettings) (ch:Interaction) dispatch =
        async {  
            try
                ...
                let sttngs = GenUtils.toRequestSettings ch.Parameters
                let fn = k.CreateSemanticFunction(Prompts.QnA.refineQuery,requestSettings=sttngs)              
                ...
                let query = ctx'.GetValue<string>()
                dispatch (Srv_Ia_Notification (ch.Id,$"Searching with: {query}"))
                let docs = 
                    cogMems
                    |> AsyncSeq.ofSeq
                    |> AsyncSeq.collect(fun cogMem -> 
                        let k = (GenUtils.baseKernel parms ch).WithMemory(cogMem).Build()
