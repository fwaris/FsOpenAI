...
open Microsoft.SemanticKernel.TemplateEngine.Basic
...
"""
    ...
    let queryPrompt parms ch query =
        task {
            ...
            let sttngs = GenUtils.toRequestSettings {ch.Parameters with Temperature=0.0; MaxTokens=500}
            let fn = k.CreateSemanticFunction(docQuery,requestSettings=sttngs)              
            let ctx = k.CreateNewContext()
            ctx.Variables.Set("document", query)      
...
"""
    ...
    let summarizeDocument parms ch document =
        task {
            let k = (GenUtils.baseKernel parms ch).Build()
            let sttngs = GenUtils.toRequestSettings {ch.Parameters with Temperature=0.0}
            let fn = k.CreateSemanticFunction(summarizePrompt,requestSettings=sttngs)              
            let ctx = k.CreateNewContext()
            ctx.Variables.Set("INPUT",document)         
            let eng = BasicPromptTemplateEngine()
            let! str = eng.RenderAsync(summarizePrompt,ctx)            
            let! ctx' = fn.InvokeAsync(ctx) |> Async.AwaitTask            
            return ctx'.GetValue<string>()            
        }
    ...
    let continueAnswerQuestion parms ch document memories dispatch combinedSearch =
        task {
            ...
            let k = (GenUtils.baseKernel parms ch).Build()
            let ctx = k.CreateNewContext()
            ctx.Variables.Set("document",document)            
            ctx.Variables.Set("searchResults",combinedSearch)
            ctx.Variables.Set("date",DateTime.Now.ToShortDateString())
            ctx.Variables.Set("question",question)
            let eng = BasicPromptTemplateEngine()
            let! renderedPrompt = eng.RenderAsync(prompt,ctx)            
            ...
            try
                ...
                let query = 
                    if dbag.SearchWithOrigText then 
                        ...
                        match Interaction.searchQuery ch with Some q -> q | _ -> failwith "no search terms found"
