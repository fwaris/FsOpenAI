    ...
    let processWebChat (parms:ServiceSettings) (ch:Interaction) dispatch =         
       async {
            try     
                ...
                //let bingConnector = new BingConnector(parms.BING_ENDPOINT.Value.API_KEY);
                //let bing = new WebSearchEnginePlugin(bingConnector);
                //let _ = kernel.ImportFunctions(bing, "bing");
                let sttngs = OpenAIRequestSettings(MaxTokens=ch.Parameters.MaxTokens, Temperature=ch.Parameters.Temperature)                
                let oracle = kernel.CreateSemanticFunction(SemanticFunction,requestSettings=sttngs)
                let question = Interaction.lastNonEmptyUserMessageText ch
                let ctxVars = ContextVariables(question)
                ctxVars.Set("externalInformation",String.Empty)
                dispatch (Srv_Ia_Notification (ch.Id,"Querying model first"))
                let! rslt =  kernel.RunAsync(oracle, ctxVars) |> Async.AwaitTask
                ...
                if searchIndicated answer then 
                    ...
                    ctx.Variables.Set("input",question)
                    ctx.Variables.Set("externalInformation",information)
                    let! prompt = kernel.PromptTemplateEngine.RenderAsync(SemanticFunctionNoCommand,ctx) |> Async.AwaitTask
