                            ...
                            comp<MudSelect<Backend>> {
                                ...
                                for b in backends do
                                    comp<MudSelectItem<Backend>> {
                                        "IsSelected" => (b = chat.Parameters.Backend)
                                        "Value" => b
                                        text (string b)
                                    }
                            }
                            ...
                            | Some bag -> 
                                comp<MudTooltip> {
                                    ...
                                    comp<MudSwitch<bool>> {
                                        ...
                                        "Label" => "Hybrid Search"
                                        "Checked" => bag.HybridSearch
                                        attr.callback "CheckedChanged" (fun (e:bool) -> dispatch (Ia_UpdateQaBag (chat.Id,{bag with HybridSearch = e})))
                                    }
                                }
                                comp<MudSlider<int>> {
                                    "Class" => "px-4"
...
type ChatSettingsView() =
    ...
    override this.View mdl (dispatch:Message -> unit) =
        let chat,model = mdl
        let settingsOpen = Update.TmpState.chatSettingsOpen chat.Id model
        concat {
            comp<MudIconButton> { 
