...
type QuestionView() =
    ...
    override this.View model dispatch =
        let selChat = Update.selectedChat model
        let dbag = selChat |> Option.map (fun c -> c.InteractionType) |> Option.bind (function DocQA d -> Some d | _-> None)
        concat {
                    ...
                    concat {
                        comp<MudPaper> {
                            ...
                            comp<MudIconButton> {
                                ...
                                on.click(fun e -> 
                                    selChat 
                                    |> Option.iter(fun ch -> dispatch (Ia_ClearChat (ch.Id,""))))
                            }
                            match dbag with
                            | Some dbag -> 
                        ...
                        comp<MudIconButton> {
                            ...
                            "Disabled" => (selChat |> Option.map(Interaction.isReady>>not) |> Option.defaultValue true)
                            "Size" => Size.Medium
                            on.click(fun e -> 
                                questionRef.Value
                                |> Option.iter(fun m -> 
