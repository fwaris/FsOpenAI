...
type DocumentViewPopup() =
    ...
    override this.View m dispatch =
        ...
        let textColor = match dbag.Document.Status with 
                        ...
                        | _           -> Color.Warning
        comp<MudPopover> {
            "Open" => Update.TmpState.isDocDetailsOpen chat.Id model
            "Style" => "width:50%;"
            ...
            comp<MudForm> {
                ...
                comp<MudTextField<string>> {
                    ...
                    "Text" => match dbag.Document.DocumentRef with Some f -> Path.GetFileName(f.Name) | None -> null
                    "ReadOnly" => true
                }                
                comp<MudTextField<string>> {
                    "Class" => "d-flex flex-grow-1 ma-2"
        ...
        comp<MudPaper> {
            ...
            comp<MudFileUpload<IBrowserFile>> {
                ...
                attr.fragmentWith "ButtonTemplate" (fun (t:FileUploadButtonTemplateContext<IBrowserFile>) -> 
                    comp<MudButton> {
                        "Disabled" => (Update.Auth.isAuthorized model |> not)
                        "Variant" => Variant.Filled
                        "Color" => Color.Primary
                        attr.``for`` inputId                           
                        "HtmlTag" => "label"                                                                
            ...
            ecomp<DocumentViewPopup,_,_> (dbag,chat,model) dispatch {attr.empty()}
        }
